...
def fetch_occupation(url):
    try:
        ...
        web_helper.load_page()
        try:
            occupation_present=web_helper.click_on_borough_button()
            df_occupations,location_text = web_helper.get_occupation_data(occupation_present)
...
def run_plot_google_maps(demo_df,df_restaurants,df_pubs,df_household_income,df_neighbourhood_income, full_address,df_occcupation,occupation_location_text,connectivity_df,stations_df,df_universities,df_tourists,df_high_schools,df_shopping_mall,postcode):
    ...
    plotter.run_google_maps(demo_df,df_restaurants,df_pubs,df_household_income,df_neighbourhood_income,full_address,df_occcupation,occupation_location_text,connectivity_df,stations_df,df_universities,df_tourists,df_high_schools,df_shopping_mall,postcode)
...
async def process_postcode(postcode: str):
    ...
    try:
        ...
        # Generate plot
        await loop.run_in_executor(executor, run_plot, demo_df, df_restaurants, df_pubs,df_household_income,df_neighbourhood_income,full_address,df_occcupation,occupation_location_text,connectivity_df,stations_df,postcode)
...
# Define the endpoint for average prediction from database 
@app.get("/process_google_maps/")
async def get_google_maps_data(postcode: str):

    # loop = asyncio.get_event_loop()
    # df_universities,df_tourists,df_high_schools,df_shopping_mall
    ...
    try:
        ...
        # Generate plot
        await loop.run_in_executor(executor, run_plot_google_maps, demo_df, df_restaurants, df_pubs,df_household_income,df_neighbourhood_income,full_address,df_occcupation,occupation_location_text,connectivity_df,stations_df,df_universities,df_tourists,df_high_schools,df_shopping_mall,postcode)
